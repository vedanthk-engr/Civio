...
def _try_postgres_with_timeout(dsn: str) -> None:
    ...
    def _connect():
        ...
        print('[database] [OK] Postgres connected (primary)')
    ...
    if t.is_alive():
        ...
        _cancelled.set()
        print(
            f'[database] [ERROR] Postgres did not connect within {_PG_TIMEOUT_S}s '
    ...
    if _result['error'] is not None:
        print(
            f'[database] [ERROR] Postgres connection failed: '
    ...
    try:
        ...
        try:
            ...
            if '429' in err_str or 'quota' in err_str.lower():
                ...
                print(f'[database] [WARNING] Firebase quota exceeded on smoke-test: {read_err}')
        ...
        print('[database] [OK] Firebase Firestore connected (fallback)')
        _seed_firebase_if_empty()

    except Exception as fb_err:
        print(f'[database] [ERROR] Firebase unavailable: {type(fb_err).__name__}: {fb_err}')
...
# ═══════════════════════════════════════════════════════
#  CIVIO PORT: GAMIFICATION & FORECASTING HELPER FUNCTIONS
# ═══════════════════════════════════════════════════════

_SEED_QUESTS = [
    {
        "id": "q1",
        "title": "Night Watch Patrol",
        "description": "Report 3 broken streetlights to restore light to your streets.",
        "category": "streetlight",
        "action": "REPORT",
        "target": 3,
        "reward_xp": 250,
        "reward_badge": "Light Keeper"
    },
    {
        "id": "q2",
        "title": "Water Guardian",
        "description": "Verify 5 active water leaks to prevent water wastage.",
        "category": "water",
        "action": "VERIFY",
        "target": 5,
        "reward_xp": 200,
        "reward_badge": "Water Guardian"
    },
    {
        "id": "q3",
        "title": "Clean Sweep Initiative",
        "description": "Report 4 garbage piles to clean up local parks.",
        "category": "garbage",
        "action": "REPORT",
        "target": 4,
        "reward_xp": 300,
        "reward_badge": "Eco Sentinel"
    }
]

def init_user_profiles():
    with _state['lock']:
        if 'user_profiles' not in _state or not _state['user_profiles']:
            _state['user_profiles'] = {}
            for user in _USERS:
                _state['user_profiles'][user] = {
                    'username': user,
                    'xp': random.randint(100, 1500),
                    'level': 1,
                    'badges': ['First Responder'] if user != 'anonymous' else [],
                    'daily_streak': random.choice([0, 1, 2, 3]),
                    'trust_score': random.randint(70, 95) if user != 'anonymous' else 50,
                    'issues_reported': 0,
                    'issues_verified': 0,
                    'quests_progress': {q['id']: 0 for q in _SEED_QUESTS},
                    'completed_quests': []
                }
                # Set level based on XP
                _state['user_profiles'][user]['level'] = 1 + (_state['user_profiles'][user]['xp'] // 500)
...
def get_user_profile(username):
    ...
    if username not in _state['user_profiles']:
        _state['user_profiles'][username] = {
            'username': username,
            'xp': 0,
            'level': 1,
            'badges': [],
    ...
    return _state['user_profiles'][username]

def get_quests(username):
    profile = get_user_profile(username)
    quests_list = []
    for q in _SEED_QUESTS:
        progress = profile['quests_progress'].get(q['id'], 0)
        completed = q['id'] in profile['completed_quests']
        quests_list.append({
            **q,
            'progress': progress,
            'completed': completed
...
def trigger_quest_action(username, action, category):
    profile = get_user_profile(username)
    ...
    if action == 'REPORT':
        ...
        profile['issues_verified'] += 1
        xp_gained += 20

    # Process quests progress
    for q in _SEED_QUESTS:
        qid = q['id']
        if qid in profile['completed_quests']:
            continue
        
        # Match action and category
        if q['action'] == action and q['category'] == category:
            profile['quests_progress'][qid] = profile['quests_progress'].get(qid, 0) + 1
            if profile['quests_progress'][qid] >= q['target']:
                profile['completed_quests'].append(qid)
                xp_gained += q['reward_xp']
                if q['reward_badge'] not in profile['badges']:
                    profile['badges'].append(q['reward_badge'])
                completed_quest_title = q['title']
    ...
    return {
        ...
        'new_level': new_level,
        'completed_quest': completed_quest_title
    }

def get_leaderboard(area=None):
    if 'user_profiles' not in _state or not _state['user_profiles']:
        init_user_profiles()
    
    profiles = list(_state['user_profiles'].values())
    profiles.sort(key=lambda p: p['xp'], reverse=True)
    return profiles

def get_decay_forecasts():
    issues = get_issues(limit=500)
    area_stats = {}
    ...
    for issue in issues:
        ...
        if tag == 'pothole': stats['potholes'] += 1
        elif tag == 'water': stats['water'] += 1
        elif tag == 'sewage': stats['sewage'] += 1
    ...
    for area, coords in AREA_COORDS.items():
        stats = area_stats[area]
        # Calculate decay risk score
        score = 25 # base
        score += stats['open'] * 6
        score += stats['escalated'] * 12
        ...
        counts = [('Road damage & potholes', stats['potholes']), ('Water pipeline leakage', stats['water']), ('Sewage blockages', stats['sewage'])]
        counts.sort(key=lambda x: x[1], reverse=True)
        top_risk = counts[0][0] if counts[0][1] > 0 else 'General infrastructure weathering'
    ...
    forecasts.sort(key=lambda f: f['score'], reverse=True)
    return forecasts
