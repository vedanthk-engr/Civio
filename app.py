...
from database import (
    ...
    get_user_profile, get_quests, get_leaderboard, trigger_quest_action,
    get_decay_forecasts,
)
...
def report_api():
    ...
    try:
        ...
        try:
            trigger_quest_action(user, 'REPORT', tag)
        except Exception as qe:
            print(f'[report] quest trigger failed: {qe}')
    except Exception as e:
...
def upvote_api(issue_id):
    ...
    if action == 'added':
        ...
        if issue:
            try:
                trigger_quest_action(user, 'VERIFY', issue.get('tag'))
            except Exception as qe:
                print(f'[upvote] quest verify trigger failed: {qe}')
            if not issue.get('escalated') and issue.get('upvotes', 0) >= CROWD_ESCALATION_THRESHOLD:
...
def public_stats():
    ...
    # Contractor SLA audit calculations (Civio port)
    CONTRACTOR_MAP = {
        'pothole': "Delhi Highway Dev Corp (DHDC)",
        'water': "Jal Board Infrastructure Ltd",
        'sewage': "Hydro-Tech Drains & Sewers",
        'streetlight': "Delhi Power & Lighting Consortium",
        'garbage': "EcoClean Waste Solutions",
        'electricity': "BSES Rajdhani Power Ltd",
        'tree': "Delhi Forestry & Parks Agency",
        'traffic': "Metro Traffic Signal Engineers",
        'noise': "Municipal Environmental Control",
        'other': "General Civic Maintenance Works"
    }

    contractor_stats = {}
    for tag, name in CONTRACTOR_MAP.items():
        contractor_stats[name] = {'name': name, 'tag': tag, 'assigned': 0, 'breaches': 0}

    for i in issues:
        tag = i.get('tag') or 'other'
        c_name = CONTRACTOR_MAP.get(tag, CONTRACTOR_MAP['other'])
        stats_entry = contractor_stats[c_name]
        stats_entry['assigned'] += 1
        
        sla = calculate_sla(i)
        if sla['sla_state'] == 'overdue' or i.get('status') == 'escalated':
            stats_entry['breaches'] += 1

    contractor_audit = []
    for name, s in contractor_stats.items():
        assigned = s['assigned']
        breaches = s['breaches']
        rate = round(breaches / assigned * 100, 1) if assigned else 0.0
        contractor_audit.append({
            ...
            'failureRate': rate,
            'flaggedForAudit': rate > 25.0 and assigned > 2
        })
    contractor_audit.sort(key=lambda x: x['failureRate'], reverse=True)
...
# ═══════════════════════════════════════════════════════
#  CIVIO PORT ROUTES: GAMIFICATION, DECAY & BUDGET
# ═══════════════════════════════════════════════════════

@app.route('/quests')
def quests_dashboard():
    username = session.get('user') or 'anonymous'
    profile = get_user_profile(username)
    quests_list = get_quests(username)
    leaderboard = get_leaderboard()
    return render_template('quests.html', profile=profile, quests=quests_list, leaderboard=leaderboard, **_common_ctx())

@app.route('/api/gamification/progress', methods=['POST'])
def gamification_progress():
    data = request.get_json(silent=True) or {}
    username = data.get('username') or session.get('user') or 'anonymous'
    action = data.get('action') # 'REPORT' or 'VERIFY'
    category = data.get('category')
    res = trigger_quest_action(username, action, category)
    return jsonify(res)
...
@app.route('/api/intelligence/decay')
def intelligence_decay():
    return jsonify(get_decay_forecasts())

@app.route('/api/intelligence/budget')
def intelligence_budget():
    ward = request.args.get('ward', 'Connaught Place')
    try:
    ...
    # Filter issues by ward and open status
    ward_issues = []
    for x in issues:
        if x.get('area') != ward:
            continue
        if x.get('status') in ['resolved', 'duplicate']:
            continue
        ward_issues.append(x)
        
    issue_count = len(ward_issues)
    
    def get_estimated_cost(issue):
    ...
    immediate_cost_inr = sum(get_estimated_cost(x) for x in ward_issues)
    
    # Scale cost to Cr for municipal levels
    immediate_cost_cr = round(immediate_cost_inr * 15.0 / 10000000, 3)
    if immediate_cost_cr == 0:
        immediate_cost_cr = 0.05 # default fallback if zero issues
    ...
    # Taxpayer savings: early repair saves money, say 80% of losses is saved
    tax_savings_cr = round(loss_cr * 0.8, 3)
    
    explanation = (
        f"Early intervention in {ward} for {issue_count} unresolved issues prevents compounded damage. "
        f"Deferring repairs for {horizon} months will increase costs from ₹{immediate_cost_cr:.2f} Cr to ₹{deferred_cost_cr:.2f} Cr, "
        f"resulting in an avoidable loss of ₹{loss_cr:.2f} Cr in taxpayer funds due to subgrade erosion and secondary infrastructure damage."
    )
    ...
    return jsonify({
        'summary': {
            ...
            'loss': loss_cr,
            'tax_savings': tax_savings_cr
        },
        'explanation': explanation
    })

@app.route('/budget')
def budget_simulator():
    return render_template('budget.html', **_common_ctx())
