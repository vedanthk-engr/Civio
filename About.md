...
Stack (AI-Driven & Evaluator-Friendly)
Frontend: HTML5 + Bootstrap + Custom Leaflet.js Maps + Jinja2 Templates

Backend: FastAPI (Python) on Cloud Run / Render

Database: Firestore, SQLite (local fallback), PostgreSQL

AI: Gemini 2.0 Flash (vision triage, chat agent, resolution loops), Vertex AI (decay forecasting)

Maps: Leaflet Maps, CartoDB Voyager tiles (default), MapTiler (optional custom layers)

Notifications: Resend (email alerts), Twilio (outbound WhatsApp updates)
